...
static char g_hostname[32] = {'a','u','r','o','r','a','\0'};
...
static char g_backend_text[64] = {'A','u','r','o','r','a','N','e','t',' ','+',' ','P','C','I',' ','s','c','a','n','\0'};
...
static void network_reset_runtime_state(void) {
    ...
    copy_limited(g_backend_text, "AuroraNet + PCI scan", sizeof(g_backend_text));
}

static void update_backend_name(void) {
    if (g_driver_active != 0) {
        copy_limited(g_backend_text, "AuroraNet + e1000 pilot driver", sizeof(g_backend_text));
        return;
    }
    if (g_nic_count > 0U) {
        copy_limited(g_backend_text, "AuroraNet + PCI scan (use netup for e1000)", sizeof(g_backend_text));
        return;
    }
    copy_limited(g_backend_text, "AuroraNet + PCI scan", sizeof(g_backend_text));
}
        ...
        for (slot = 0U; slot < 32U; ++slot) {
            ...
            for (func = 0U; func < 8U; ++func) {
                ...
                if (vendor == 0x10ECU && device == 0x8139U) {
                    nic_add(vendor, device, bus, slot, func, bar0, irq_line, "Realtek RTL8139", "planned later: a lightweight rtl8139 driver");
                    continue;
                }
                ...
                if (vendor == 0x1022U && device == 0x2000U) {
                    nic_add(vendor, device, bus, slot, func, bar0, irq_line, "AMD PCnet/Am79C973", "older virtual NIC, possible future fallback");
                    continue;
                }
                if (subclass == 0x00U) {
...
int network_ping(const char* target, char* out, size_t max) {
    ...
    append_limited(out, max, target);
    if (g_driver_active != 0) {
        append_limited(out, max, ": e1000 driver is active, link ");
...
int network_load_persistent(void) {
    ...
    if (header->magic != NET_PERSIST_MAGIC || header->version != NET_PERSIST_VERSION) {
        return -1;
    }
...
int network_send_probe(void) {
    ...
    static const unsigned char payload[] = {
        'A','u','r','o','r','a','O','S',' ','e','1','0','0','0',' ','p','r','o','b','e',' ','f','r','a','m','e','\0'
    };
