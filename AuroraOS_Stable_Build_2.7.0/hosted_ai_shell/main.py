...
VERSION = "AuroraOS Stable Build 2.7.0"
...
class AuroraShell:
    def __init__(self) -> None:
        self.state = SystemState(
            apps={
                "lumen": App("lumen", True, True, "Nano-artiger Texteditor", "Nano-like text editor"),
                "files": App("files", True, True, "Dateibereich", "File area"),
                "settings": App("settings", True, True, "Grundlegende Einstellungen", "Basic settings"),
                "network": App("network", True, True, "Netzwerk- und Adapteruebersicht", "Network and adapter view"),
                "monitor": App("monitor", True, True, "Systemueberblick", "System overview"),
                "browser": App("browser", False, False, "Web-Platzhalter", "Web placeholder"),
                "desktop": App("desktop", False, False, "Desktop-Platzhalter", "Desktop placeholder"),
                "paint": App("paint", False, False, "Zeichen-Platzhalter", "Drawing placeholder"),
            }
        )
        self.passwords = {"guest": "guest", "system": "aurora"}
    ...
    def t(self, de: str, en: str) -> str:
        return de if self.state.language == "de" else en
    ...
    def prompt(self) -> str:
        ...
        return f"aurora({self.state.language})@{self.state.current_user}{tail}{'# ' if self.state.system_mode else '> '}"

    def print_banner(self) -> None:
        print("============================================================")
        print(VERSION)
        print("Thank you for using AuroraOS!")
    ...
    def run(self) -> None:
        self.print_banner()
        print(self.t("Shell bereit. Apps, Nutzer, AuroraFS, Netzwerk, e1000-Grundlage und die neue Settings-App sind aktiv.", "Shell ready. Apps, users, AuroraFS, network, e1000 groundwork and the new settings app are active."))
        self.cmd_quickstart([])
        while self.running:
            try:
            ...
            if cmd in self.commands:
                ...
                print(self.t("Nicht direkt erkannt. Ich versuche die AI-Hilfe ...", "Not recognized directly. I will try AI help ..."))
                self.cmd_ai(parts)

    def _needs_system(self) -> bool:
        if self.state.system_mode:
            return False
        print(self.t("Dafuer brauchst du System-Rechte. Nutze 'elevate <passwort>'.", "You need system rights. Use 'elevate <password>'."))
        return True

    def cmd_help(self, _: List[str]) -> None:
        rows = [
            ("help", "Zeigt diese Hilfe.", "Shows this help."),
            ("quickstart", "Zeigt einen schnellen Einstieg.", "Shows a quick start."),
            ("status", "Kurzer Systemueberblick.", "Short system overview."),
            ("diag", "Kurze Diagnose fuer System und Netzwerk.", "Short diagnosis for system and network."),
            ("whoami", "Zeigt den aktuellen Benutzer.", "Shows the current user."),
            ("users", "Zeigt Nutzer und System-Modus.", "Shows users and system mode."),
            ("fs", "Erklaert AuroraFS kurz.", "Explains AuroraFS briefly."),
            ("network", "Zeigt Netzwerk-Status.", "Shows network status."),
            ("nic", "Zeigt erkannte Netzwerkadapter.", "Shows detected network adapters."),
            ("netup", "Startet den e1000-Pilottreiber.", "Starts the e1000 pilot driver."),
            ("netprobe", "Sendet einen kleinen Rohdaten-Test.", "Sends a small raw packet test."),
            ("mac", "Zeigt die aktive MAC-Adresse.", "Shows the active MAC address."),
            ("app list", "Zeigt eingebaute und optionale Apps.", "Shows built-in and optional apps."),
            ("app info <name>", "Zeigt Details zu einer App.", "Shows app details."),
            ("cmd new <name>", "Erstellt einen eigenen Befehl.", "Creates your own command."),
            ("prog info <name>", "Zeigt Rechte und Vertrauen eines Programms.", "Shows permissions and trust of a program."),
            ("which <name>", "Zeigt, woher ein Befehl kommt.", "Shows where a command comes from."),
            ("app install <name>", "Installiert optionale Apps im System-Modus.", "Installs optional apps in system mode."),
            ("app run <name>", "Startet eine App oder ihren Platzhalter.", "Starts an app or its placeholder."),
            ("settings", "Oeffnet die neue Einstellungszentrale mit Kategorien.", "Opens the new categorized settings center."),
            ("ping 127.0.0.1", "Testet Loopback.", "Tests loopback."),
            ("elevate <pw>", "Aktiviert den root-aehnlichen System-Modus.", "Activates the root-like system mode."),
            ("about", "Erklaert kurz die Idee hinter AuroraOS.", "Explains the idea behind AuroraOS."),
            ("ai <text>", "Nimmt normale Sprache entgegen.", "Accepts natural language."),
            ("exit", "Beendet die Demo.", "Exits the demo."),
        ]
        print(self.t("AuroraOS Hilfe", "AuroraOS help"))
        for cmd, de, en in rows:
            print(f"  {cmd:18} - {de if self.state.language == 'de' else en}")
    ...
    def cmd_quickstart(self, _: List[str]) -> None:
        ...
        print(self.t("Schnellstart:", "Quick start:"))
        for item in lines:
            print(f"  {item}")

    def cmd_status(self, _: List[str]) -> None:
        installed = sum(1 for app in self.state.apps.values() if app.installed)
        print(self.t("Kurzueberblick:", "Quick overview:"))
        print(f"  {self.t('Benutzer', 'User')}: {self.state.current_user} [{'system' if self.state.system_mode else 'user'}]")
        print(f"  {self.t('Ort', 'Path')}: {self.state.current_path}")
        print(f"  {self.t('Freier Speicher', 'Free memory')}: {self.state.heap_free} bytes")
        print(f"  {self.t('Netzwerk', 'Network')}: {self.state.hostname} / {self.state.ip} / NICs={len(self.state.nics)} / Driver={self.state.driver}")
        print(f"  {self.t('Expertenmodus', 'Expert mode')}: {'on' if self.state.expert_mode else 'off'}")
        print(f"  {self.t('Installierte Apps', 'Installed apps')}: {installed}")
        print(f"  {self.t('Ticks seit Start', 'Ticks since boot')}: {self.state.ticks}")

    def cmd_diag(self, _: List[str]) -> None:
        print(self.t("Diagnose:", "Diagnostics:"))
    ...
    def cmd_history(self, _: List[str]) -> None:
        print(self.t("Letzte Befehle:", "Recent commands:"))
        if not self.state.history:
            print(self.t("  Noch keine Befehle.", "  No commands yet."))
            return
        for idx, item in enumerate(self.state.history, 1):
            print(f"  {idx}. {item}")

    def cmd_whoami(self, _: List[str]) -> None:
        print(f"{self.t('Aktiver Benutzer', 'Current user')}: {self.state.current_user}")
        print(f"Home: {self.homes[self.state.current_user]}")
        print(self.t("Sitzung: System-Modus aktiv" if self.state.system_mode else "Sitzung: Normale Rechte", "Session: system mode active" if self.state.system_mode else "Session: normal rights"))

    def cmd_users(self, _: List[str]) -> None:
        print(self.t("Bekannte Benutzer:", "Known users:"))
        print("  - guest [user] /home/guest")
        print(self.t("  - system-mode [special] root-aehnlich per elevate <passwort>", "  - system-mode [special] root-like via elevate <password>"))

    def cmd_login(self, args: List[str]) -> None:
        if not args or args[0] != "guest":
            print(self.t("Derzeit gibt es in der Demo nur 'guest'.", "At the moment the demo only has 'guest'."))
            return
        self.state.current_user = "guest"
        self.state.current_path = self.homes["guest"]
        self.state.system_mode = False
        print(self.t("Benutzer gewechselt zu guest", "Switched user to guest"))
    ...
    def cmd_fs(self, _: List[str]) -> None:
        print(self.t("AuroraFS im Ueberblick:", "AuroraFS overview:"))
        print("  /system  /home  /apps  /temp")
        print(self.t("  UNIX-inspiriert, aber ein eigenes AuroraFS.", "  UNIX-inspired, but its own AuroraFS."))
    ...
    def cmd_settings(self, args: List[str]) -> None:
        ...
        while True:
            if page == "home":
                ...
                print(f"  1. {self.t('Allgemein', 'General'):22} : {('Deutsch / DE' if self.state.language == 'de' else 'English / EN')}")
            ...
            if page == 'general':
                ...
                print(f"  1. {self.t('Sprache', 'Language'):22} : {'Deutsch' if self.state.language == 'de' else 'English'}")
                ...
                if choice in {'', '5', 'q', 'back'}:
                    ...
                    self.state.language = 'en' if self.state.language == 'de' else 'de'
                    print(self.t("Sprache umgeschaltet.", "Language switched."))
                elif choice == '2':
    ...
    def cmd_lang(self, args: List[str]) -> None:
        if not args or args[0] not in {"de", "en"}:
            print(self.t("Nutze: lang <de|en>", "Use: lang <de|en>"))
            return
        self.state.language = args[0]
        print("Sprache gesetzt: Deutsch" if args[0] == "de" else "Language set: English")

    def cmd_network(self, _: List[str]) -> None:
        print(self.t("AuroraNet im Ueberblick:", "AuroraNet overview:"))
        print(f"  Backend: {'AuroraNet + e1000 pilot driver' if self.state.driver != 'off' else 'AuroraNet + PCI scan'}")
    ...
    def cmd_app(self, args: List[str]) -> None:
        if not args or args[0] == "list":
            ...
            for app in self.state.apps.values():
                ...
                desc = app.description_de if self.state.language == "de" else app.description_en
        ...
        if action == "install":
            ...
            print(f"Description: {app.description_de if self.state.language == 'de' else app.description_en}")
    ...
    def cmd_about(self, _: List[str]) -> None:
        print(self.t("AuroraOS soll leicht verstaendlich wie Windows und modular wie Linux sein.", "AuroraOS aims to be easy to understand like Windows and modular like Linux."))
        print(self.t("Gerade gibt es Shell, AuroraFS-Grundlage, Rechte, App-Modell, eine menueartige Settings-App, Lumen, Persistenz und einen ersten e1000-Pilottreiber.", "Right now it has a shell, AuroraFS groundwork, rights, an app model, a menu-like settings app, Lumen, persistence and a first e1000 pilot driver."))
        print(CREDIT)
    ...
    def cmd_ai(self, args: List[str]) -> None:
        ...
        if any(word in text for word in ["user", "benutzer", "rechte", "system"]):
            print(self.t("AI-Tipp: Nutze whoami, users, elevate aurora oder diag.", "AI tip: use whoami, users, elevate aurora or diag."))
            return
    ...
    def cmd_exit(self, _: List[str]) -> None:
        self.running = False
        print(self.t("AuroraOS Demo wird beendet.", "AuroraOS demo is closing."))
