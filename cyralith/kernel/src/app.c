...
int app_load_persistent(void) {
    ...
    if (header->magic != APP_PERSIST_MAGIC) {
        return -1;
    }
    if (header->version != 1U && header->version != APP_PERSIST_VERSION) {
        return -1;
    }
    if (header->version == APP_PERSIST_VERSION && header->count != (unsigned int)app_count()) {
        return -1;
    }
    if (header->version == 1U && header->count != 8U) {
        return -1;
    }
