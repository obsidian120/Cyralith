#include "gdt.h"

typedef struct {
    uint16_t limit_low;
    uint16_t base_low;
    uint8_t base_mid;
    uint8_t access;
    uint8_t granularity;
    uint8_t base_high;
} __attribute__((packed)) gdt_entry_t;

typedef struct {
    uint16_t limit;
    uint32_t base;
} __attribute__((packed)) gdt_ptr_t;

typedef struct {
    uint32_t prev_tss;
    uint32_t esp0;
    uint32_t ss0;
    uint32_t esp1;
    uint32_t ss1;
    uint32_t esp2;
    uint32_t ss2;
    uint32_t cr3;
    uint32_t eip;
    uint32_t eflags;
    uint32_t eax;
    uint32_t ecx;
    uint32_t edx;
    uint32_t ebx;
    uint32_t esp;
    uint32_t ebp;
    uint32_t esi;
    uint32_t edi;
    uint32_t es;
    uint32_t cs;
    uint32_t ss;
    uint32_t ds;
    uint32_t fs;
    uint32_t gs;
    uint32_t ldt;
    uint16_t trap;
    uint16_t iomap_base;
} __attribute__((packed)) tss_entry_t;

static gdt_entry_t g_gdt[6];
static gdt_ptr_t g_gdt_ptr;
static tss_entry_t g_tss;
static uint32_t g_kernel_stack_top = 0U;

extern void gdt_flush(uint32_t gdt_ptr_addr);
extern void tss_flush(void);

static void gdt_set_entry(uint32_t index, uint32_t base, uint32_t limit, uint8_t access, uint8_t granularity) {
    g_gdt[index].limit_low = (uint16_t)(limit & 0xFFFFU);
    g_gdt[index].base_low = (uint16_t)(base & 0xFFFFU);
    g_gdt[index].base_mid = (uint8_t)((base >> 16U) & 0xFFU);
    g_gdt[index].access = access;
    g_gdt[index].granularity = (uint8_t)(((limit >> 16U) & 0x0FU) | (granularity & 0xF0U));
    g_gdt[index].base_high = (uint8_t)((base >> 24U) & 0xFFU);
}

void gdt_init(uint32_t kernel_stack_top) {
    uint32_t tss_base = (uint32_t)&g_tss;
    uint32_t tss_limit = (uint32_t)sizeof(g_tss) - 1U;
    uint32_t i;

    for (i = 0U; i < (uint32_t)sizeof(g_tss); ++i) {
        ((uint8_t*)&g_tss)[i] = 0U;
    }

    g_kernel_stack_top = kernel_stack_top;
    g_gdt_ptr.limit = (uint16_t)(sizeof(g_gdt) - 1U);
    g_gdt_ptr.base = (uint32_t)&g_gdt[0];

    gdt_set_entry(0U, 0U, 0U, 0U, 0U);
    gdt_set_entry(1U, 0U, 0xFFFFFU, 0x9AU, 0xCFU);
    gdt_set_entry(2U, 0U, 0xFFFFFU, 0x92U, 0xCFU);
    gdt_set_entry(3U, 0U, 0xFFFFFU, 0xFAU, 0xCFU);
    gdt_set_entry(4U, 0U, 0xFFFFFU, 0xF2U, 0xCFU);
    gdt_set_entry(5U, tss_base, tss_limit, 0x89U, 0x40U);

    g_tss.ss0 = GDT_KERNEL_DATA_SELECTOR;
    g_tss.esp0 = kernel_stack_top;
    g_tss.cs = GDT_USER_CODE_SELECTOR;
    g_tss.ss = GDT_USER_DATA_SELECTOR;
    g_tss.ds = GDT_USER_DATA_SELECTOR;
    g_tss.es = GDT_USER_DATA_SELECTOR;
    g_tss.fs = GDT_USER_DATA_SELECTOR;
    g_tss.gs = GDT_USER_DATA_SELECTOR;
    g_tss.iomap_base = (uint16_t)sizeof(g_tss);

    gdt_flush((uint32_t)&g_gdt_ptr);
    tss_flush();
}

void gdt_set_kernel_stack(uint32_t kernel_stack_top) {
    g_kernel_stack_top = kernel_stack_top;
    g_tss.esp0 = kernel_stack_top;
}

uint32_t gdt_kernel_stack_top(void) {
    return g_kernel_stack_top;
}
